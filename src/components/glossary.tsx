...
export default function Glossary({ items }: GlossaryProps) {
  ...
  const [manualFilteredSections, setManualFilteredSections] = useState<
    string[] | null
  >(null);
  ...
  const handleClickLetter = (letter: string) => {
    ...
    const acordionElement = document.getElementById(
      `letter-${alfabet[letterIndex]}`,
    );
    if (acordionElement) {
      const scrollContainer = document.getElementById("main-scroll-container");
      const headerHeight =
        document.getElementById("glossary-header")?.offsetHeight ?? 64;
      if (scrollContainer) {
  ...
  const itemsThatContainsSearchedTerm = () => {
    ...
    const foundedItems = alfabet.filter((letter) =>
      items[letter]?.some(
        (item) =>
          item.item.toLowerCase().includes(term) ||
          item.description.toLowerCase().includes(term),
  ...
  const navigateToMatch = (index: number) => {
    const marks = Array.from(
      document.querySelectorAll("#glossary-accordion mark"),
    ) as HTMLElement[];
    marks.forEach((m) => {
  ...
  const handleNext = () => {
    const next =
      totalMatches === 0 ? 0 : (currentMatchIndex + 1) % totalMatches;
  ...
  const handleClickSection = (letter: string) => {
    if (effectiveSections.includes(letter)) {
      setManualFilteredSections(
        effectiveSections.filter((el) => el !== letter),
      );
    } else {
  ...
  return (
    <div className="w-full overflow-x-hidden">
      <div
        className="fixed flex justify-center items-center left-0 top-7 h-35 bg-white w-full gap-4 max-md:h-50"
        id="glossary-header"
      >
        <div className="absolute left-1/2 -translate-x-1/2 top-10 z-10 w-2/3 max-sm:w-11/12 mx-auto max-w-xl flex justify-center flex-col">
            ...
            {alfabet.map((letter) => (
              ...
              >
                ...
                <AccordionContent className="flex flex-col gap-6 text-balance break-words mt-4">
                  {items[letter] &&
                    items[letter].map((item, index) => (
                      <div key={index}>
                        <h3 className="text-base font-bold mb-2 text-primary">
                          {highlightText(item.item, committedTerm)}
                        </h3>
                  ...
                  {!items[letter] && (
                    <p className="mt-4">
                      Nenhum termo encontrado para a letra "{letter}".
                    </p>
                  )}
                </AccordionContent>
              </AccordionItem>
